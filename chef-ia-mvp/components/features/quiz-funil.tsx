...
export function QuizFunil() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setFase("resultado"), 1800);
  ...
  async function enviarContato() {
    ...
    try {
      ...
      perguntas.forEach((p, i) => {
        respostasObj[p.pergunta] = respostas[i] || "";
      });
  ...
  const progresso =
    fase === "perguntas"
      ? Math.round(((passo + 1) / (total + 2)) * 100)
      : fase === "contato"
      ? Math.round(((total + 1) / (total + 2)) * 100)
      : fase === "calculando" || fase === "resultado"
      ? 100
      : 0;

  const progressoStyle = { width: progresso + "%" };
  const primeiroNome = nome.trim().split(" ")[0];
  const resultadoTitulo = nome
    ? primeiroNome + ", o Chef IA foi feito pra isso"
    : "O Chef IA foi feito pra resolver exatamente isso";

  const resultadoTexto = desafio
    ? 'Voce disse que o maior desafio hoje e: "' +
      desafio +
      '". ' +
      (ferramenta ? 'Hoje voce ainda controla tudo com "' + ferramenta.toLowerCase() + '". ' : "") +
      (tempoPerdido ? "E perde " + tempoPerdido.toLowerCase() + " por semana com isso. " : "") +
  ...
  return h(
    ...
    fase !== "intro" &&
      h(
        ...
        h(
          "div",
          { className: "h-2 w-full overflow-hidden rounded-full bg-cacau/10 dark:bg-cream/10" },
          h("div", { className: "h-full rounded-full bg-framboesa transition-all duration-300", style: progressoStyle })
        ),
        fase === "perguntas" &&
          h("p", { className: "mt-2 text-xs text-cacau/50 dark:text-cream/50" }, "Pergunta ", passo + 1, " de ", total)
      ),
    fase === "intro" &&
      h(
        Card,
        ...
        h(
          ...
          "Responda ",
          total,
          " perguntas rapidas sobre a sua confeitaria e receba um diagnostico personalizado na tela, sem compromisso."
        ),
        h(
          "div",
          { className: "mb-6 flex flex-col gap-2 text-left text-sm text-cacau/70 dark:text-cream/70" },
          h("div", null, "Vamos entender como voce precifica hoje"),
    ...
    fase === "perguntas" &&
      h(
        Card,
        null,
        h("h2", { className: "mb-2 font-display text-2xl" }, perguntas[passo].pergunta),
        perguntas[passo].ajuda && h("p", { className: "mb-6 text-xs text-cacau/50 dark:text-cream/50" }, perguntas[passo].ajuda),
        h(
          "div",
          ...
          perguntas[passo].opcoes.map((opcao) =>
            h(
              "button",
              {
                ...
                className:
                  "rounded-2xl border border-cacau/10 bg-white/60 px-4 py-3 text-left text-sm transition-all duration-150 hover:border-framboesa hover:bg-framboesa/5 dark:border-cream/10 dark:bg-cacau-soft/40 dark:hover:bg-framboesa/10",
              },
              opcao
            )
          )
        ),
        passo > 0 && h("button", { onClick: voltar, className: "mt-6 text-xs text-cacau/50 hover:underline dark:text-cream/50" }, "Voltar")
      ),
    fase === "contato" &&
      h(
        Card,
        null,
        h("h2", { className: "mb-2 font-display text-2xl" }, "Falta pouco! Pra onde enviamos seu diagnostico?"),
        h(
          "p",
          { className: "mb-6 text-sm text-cacau/70 dark:text-cream/70" },
          "Assim voce ve na tela o resultado personalizado e a condicao de fundadora, e a gente consegue te ajudar por WhatsApp se precisar."
        ),
        h(
          "div",
          { className: "mb-4" },
          h(Label, { htmlFor: "quiz-nome" }, "Seu nome"),
          h(Input, {
            id: "quiz-nome",
            value: nome,
        ...
        h(
          "div",
          { className: "mb-6" },
          h(Label, { htmlFor: "quiz-whatsapp" }, "Seu WhatsApp"),
          h(Input, {
            id: "quiz-whatsapp",
            value: whatsapp,
        ...
        h(
          Button,
          { className: "w-full", onClick: enviarContato, disabled: enviando || !nome.trim() || !whatsapp.trim() },
          enviando ? "Enviando..." : "Ver meu resultado"
        ),
        h("button", { onClick: voltar, className: "mt-6 text-xs text-cacau/50 hover:underline dark:text-cream/50" }, "Voltar")
      ),
    fase === "calculando" &&
      h(
        Card,
        { className: "text-center" },
        h("div", { className: "mx-auto mb-6 h-10 w-10 animate-spin rounded-full border-2 border-cacau/15 border-t-framboesa" }),
        h("h2", { className: "mb-2 font-display text-xl" }, "Calculando o diagnostico do seu negocio..."),
        h("p", { className: "text-sm text-cacau/60 dark:text-cream/60" }, "Ja estamos quase la.")
      ),
    fase === "resultado" &&
      h(
        Card,
        ...
        h(Link, { href: "/cadastro" }, h(Button, { className: "w-full" }, "Quero garantir minha vaga de fundadora"))
      )
  );
}
